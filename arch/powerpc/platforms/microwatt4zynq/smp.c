...
/*
 * SMP support functions for Microwatt4Zynq
 *
 * Derived from arch/powerpc/platforms/microwatt/smp.c, which is:
...
static void __init microwatt4zynq_smp_probe(void)
{
	xics_smp_probe();
}

static void microwatt4zynq_smp_setup_cpu(int cpu)
{
	if (cpu != 0)
		xics_setup_cpu();
}

static struct smp_ops_t microwatt4zynq_smp_ops = {
	.probe		= microwatt4zynq_smp_probe,
	.message_pass	= NULL,		/* Use smp_muxed_ipi_message_pass */
	.kick_cpu	= smp_generic_kick_cpu,
	.setup_cpu	= microwatt4zynq_smp_setup_cpu,
};
...
void __init microwatt4zynq_init_smp(void)
{
	...
	smp_ops = &microwatt4zynq_smp_ops;
